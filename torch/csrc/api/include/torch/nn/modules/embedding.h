...
class TORCH_API EmbeddingBagImpl : public torch::nn::Cloneable<EmbeddingBagImpl> {
 public:
  ...
  // void forward(uint64_t indices_ptr, uint64_t offsets_ptr, uint64_t indices_len_ptr, uint64_t nr_batches_ptr, uint64_t final_results_ptr, uint64_t num_of_tables, uint64_t dpu_set_ptr, bool lookup_mode, bool use_dpu);
  Tensor forward(const Tensor& input, const Tensor& offsets = {}, const Tensor& per_sample_weights = {}, int64_t num_of_tables = 0, int64_t dpu_set_ptr = 0, bool use_dpu = false, int64_t final_results_ptr = 0, int64_t indices_ptr = 0, int64_t offsets_ptr = 0, int64_t indices_len_ptr = 0, int64_t offsets_len_ptr = 0, int64_t latency_print = 0, int64_t nr_cols_ptr = 0);
