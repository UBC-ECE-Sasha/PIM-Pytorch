...
class TORCH_API EmbeddingBagImpl : public torch::nn::Cloneable<EmbeddingBagImpl> {
 public:
  ...
  Tensor forward(const Tensor& input, const Tensor& offsets = {}, const Tensor& per_sample_weights = {}, int64_t num_of_tables = 0, int64_t dpu_set_ptr = 0, bool lookup_mode = false, bool use_dpu = false, int64_t final_results_ptr = 0);
